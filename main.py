...
def Banner(pdf,json_data,c_MoneyS,money_signData,user_data):
    ...
    # pdf.image(join(cwd,'assets', 'images','money_sign_png',c_MoneyS+'.png'),px2MM(1120), px2MM(12), px2MM(736), px2MM(1100))
    # pdf.rect(0, 0, px2MM(1120), px2MM(1080), 'F')
...
def libility_management_1(pdf,json_data,c_MoneyS,money_signData):
    ...
    pdf.set_xy(px2MM(436),px2MM(text_y)) 
    val1 = str(locale.currency(aff_check["Outstanding"].iloc[-1], grouping=True))
    val1 = val1.split('.')[0]
    ...
    pdf.set_xy(px2MM(782),px2MM(text_y)) 
    val2 = str(locale.currency(aff_check["EMI"].iloc[-1], grouping=True))
    val2 = val1.split('.')[0]
